...
<body class="fix-header">
    ...
    <div class="card p-30">
        <div class="media">
            ...
            <div class="media-body media-text-right">
                <?php 
                $sql = "SELECT SUM(price) AS value_sum FROM users_orders WHERE status = 'closed'";
                $result = mysqli_query($db, $sql); 
                if ($result) {
                    $row = mysqli_fetch_assoc($result); 
                    $sum = $row['value_sum'];
                    echo '<h2>'.number_format($sum, 2).'</h2>'; // format the number to two decimal places if needed
                } else {
                    echo '<h2>0.00</h2>'; // default value or error handling
                }
                ?>
